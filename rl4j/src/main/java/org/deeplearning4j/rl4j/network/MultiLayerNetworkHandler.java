...
public class MultiLayerNetworkHandler extends PersistentNetworkHandler {
	...
	public static final String CONFIGURATION_JSON = "multiLayerNetworkHandler.json";
	 ...
	 * @param labelName       The name of the label (in {@link FeaturesLabels}) to
	...
	public void applyGradient(Gradients gradients, long batchSize) {
		...
		model.getUpdater().update(model, gradient, iterationCount, modelCounters.getEpochCount(), (int) batchSize,
	...
	public void saveTo(File file, boolean saveUpdater) throws IOException {
		...
		if (success) {
			...
			saveConfigurationTo(file, new Configuration(recurrent, labelName, gradientName, inputFeatureIdx));
		}
	}
