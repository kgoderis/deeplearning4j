...
@Value
public class DQNFactoryStdConv implements NetworkFactory {
    ...
    public NeuralNetwork[] build(int[] numInputs, int numOutputs) {
        ...
        NeuralNetConfiguration.ListBuilder confB = new NeuralNetConfiguration.Builder().seed(Constants.NEURAL_NET_SEED)
                        ...
                        .layer(0, new ConvolutionLayer.Builder(8, 8).nIn(numInputs[0]).nOut(16).stride(4, 4)
        ...
        confB.setInputType(InputType.convolutional(numInputs[1], numInputs[2], numInputs[0]));
