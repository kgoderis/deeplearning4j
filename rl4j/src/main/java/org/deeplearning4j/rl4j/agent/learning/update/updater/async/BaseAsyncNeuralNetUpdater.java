/*
 *  ******************************************************************************
 *  *
 *  *
 *  * This program and the accompanying materials are made available under the
 *  * terms of the Apache License, Version 2.0 which is available at
 *  * https://www.apache.org/licenses/LICENSE-2.0.
 *  *
 *  *  See the NOTICE file distributed with this work for additional
 *  *  information regarding copyright ownership.
 *  * Unless required by applicable law or agreed to in writing, software
 *  * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
 *  * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the
 *  * License for the specific language governing permissions and limitations
 *  * under the License.
 *  *
 *  * SPDX-License-Identifier: Apache-2.0
 *  *****************************************************************************
 */
package org.deeplearning4j.rl4j.agent.learning.update.updater.async;

import lombok.NonNull;
import org.deeplearning4j.rl4j.agent.learning.update.Gradients;
import org.deeplearning4j.rl4j.agent.learning.update.updater.NeuralNetUpdater;
import org.deeplearning4j.rl4j.network.TrainableNeuralNet;

public abstract class BaseAsyncNeuralNetUpdater<RESULT> implements NeuralNetUpdater<RESULT> {
    protected final TrainableNeuralNet threadCurrent;
    private final AsyncSharedNetworksUpdateHandler sharedNetworksUpdateHandler;

    protected BaseAsyncNeuralNetUpdater(@NonNull TrainableNeuralNet threadCurrent,
                                        @NonNull AsyncSharedNetworksUpdateHandler sharedNetworksUpdateHandler) {
        this.threadCurrent = threadCurrent;
        this.sharedNetworksUpdateHandler = sharedNetworksUpdateHandler;
    }

    @Override
    public abstract void update(RESULT dataType);

    protected void updateAndSync(Gradients gradients) {
        sharedNetworksUpdateHandler.handleGradients(gradients);
    }

    @Override
    public void synchronizeCurrent() {
        threadCurrent.copyFrom(sharedNetworksUpdateHandler.getGlobalCurrent());
    }

}
