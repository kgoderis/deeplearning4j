...
public class RecurrentNStepQLearningTest {
    ...
    @Mock
    TrainableNeuralNet threadCurrentMock;

    @Mock
    OutputNeuralNet targetMock;
    ...
    private void setup(double gamma) {
        ...
        when(targetMock.output(any(Features.class))).thenAnswer(invocation -> {
            NeuralNetOutput result = new NeuralNetOutput();
            result.put(CommonOutputNames.QValues, invocation.getArgument(0, Features.class).get(0).mul(-2.0));
            return result;
        });
